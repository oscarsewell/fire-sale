...
The full scraping module extract extensive product data from our top retailers. This is triggered when a user submits a product URL and target price through the submission form on our Dashboard, or through the Discord Bot. It is used when seeding a new product into the database.
...
- `[WEBSITE-NAME]_full_scraper.py`: Fully scrapes the product data for a given URL.
...
- `test_[WEBSITE-NAME]_full_scraper.py`: Automated unit tests verifying extraction accuracy, parser boundary behaviors, and custom exception handling under mock network requests.
...
```bash
# AWD-IT
python3 full_scraping/awd-it/awd_it_full_scraper.py

# Ebuyer
python3 full_scraping/ebuyer/ebuyer_full_scraper.py

# Overclockers
python3 full_scraping/overclockers/overclockers_full_scraper.py
```
...
```bash
pip install -r full_scraping/requirements.txt
```
...
pytest full_scraping/awd-it/
```

## Inputs and Outputs

A list of URLs is processed, and a consistent list of dictionaries is returned.
...
```json
{
  ...
  "product_name": "CORSAIR Vengeance DDR5 RAM 32GB",
