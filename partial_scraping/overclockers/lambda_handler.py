"""AWS Lambda handler that scrapes product data from URLs for overclockers products"""
import json
import logging

from overclockers_scraper import scrape_all_products
...
def lambda_handler(event, context):
    """AWS Lambda handler that scrapes overclockers URLs"""
    try:
        ...
        overclockers_products = products_by_site.get("overclockers", [])

        urls_and_ids = []
        for product_id, product_url in overclockers_products:
